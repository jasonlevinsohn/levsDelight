...
if(isset($_GET['pictureTitle1']) && isset($_GET['pictureDesc1'])) {
	...
	for($i = 1; $i < $theCounter + 1; $i++) {
		...
		$updatePrep->execute(array($updateTitle, $updateDesc, $updateId)) or die('Error executing the update');
	}
...
<!-- If we are logged in, we can change the description, otherwise no. -->
				<?php if(isset($_SESSION['SESS_MEMBER_ID']) && isset($_SESSION['SESS_FIRST_NAME'])) { 
					echo "Update the Descriptions for the pictures here: <br />  ";
					$pictureShowQueryTwo = "SELECT * FROM slideshows WHERE slideshowid = 1 and isActive = 1 ";
					...
					<?php foreach($pictureShowStatementTwo as $row) { 
							$counter++;
					?>
					<div id="updateDesc<?php echo $row['id']; ?>" style="border: 1px solid #000; margin-bottom: 20px;">
						<h3><?php echo $row['id'] . '.)' ?> </h3>
						<img src="<?php echo $row['pictureLocation']; ?>" width="100" height="100" />
						<input type="text" id="pictureTitle<?php echo $row['id']; ?>" name="pictureTitle<?php echo $row['id']; ?>" value="<?php echo $row['title']; ?>" />
						<input type="text" id="pictureDesc<?php echo $row['id']; ?>" name="pictureDesc<?php echo $row['id']; ?>" value="<?php echo $row['desc']; ?>" />
					</div>
					<?php } ?>
