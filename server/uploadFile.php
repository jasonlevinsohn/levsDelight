...
if(isset($GLOBALS["HTTP_RAW_POST_DATA"])) {
	...
	if($imageType === false) {
		...
		try {
			...
			$fileToSave = "../june2012/june2012-pictures/" . $filename;
			$fp = fopen($fileToSave, 'x');
			fwrite($fp, $unencodedData);
			fclose($fp);
			
			$pictureLocation = "june2012-pictures/" . $filename;
			...
			$insertPicturePrep->execute(array($pictureLocation, 1, 7)) or die('Error inserting picture record');
